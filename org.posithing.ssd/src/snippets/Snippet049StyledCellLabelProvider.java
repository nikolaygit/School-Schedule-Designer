...
public class Snippet049StyledCellLabelProvider {
	...
	public Composite createPartControl(Composite parent) {
		...
		final TableViewer tableViewer = new TableViewer(composite, SWT.MULTI | SWT.H_SCROLL
				| SWT.V_SCROLL);
		...
		// ignored.
		FontData[] boldFontData = getModifiedFontData(tableViewer.getTable().getFont()
	...
	private static class ExampleLabelProvider extends StyledCellLabelProvider {
		...
		private static final Image IMAGE1 = new Image(DISPLAY, DISPLAY.getSystemImage(
				SWT.ICON_WARNING).getImageData().scaledTo(IMAGE_SIZE, IMAGE_SIZE));
		private static final Image IMAGE2 = new Image(DISPLAY, DISPLAY.getSystemImage(
				SWT.ICON_ERROR).getImageData().scaledTo(IMAGE_SIZE, IMAGE_SIZE));
		...
		public void update(ViewerCell cell) {
			...
			if (element instanceof File) {
				...
				String decoration = MessageFormat.format(
						" ({0} bytes)", new Object[] { new Long(file.length()) }); //$NON-NLS-1$
