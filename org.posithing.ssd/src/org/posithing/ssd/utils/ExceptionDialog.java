 ...
 */
public class ExceptionDialog extends ExceptionIconAndMessageDialog {
	/**
	 ...
	 */
	public ExceptionDialog(Shell parentShell, String dialogTitle, String message, IStatus status, int displayMask) {
		super(parentShell);
	...
	public static int openError(Shell parentShell, String title, String message, IStatus status, int displayMask) {
		ExceptionDialog dialog = new ExceptionDialog(parentShell, title, message, status, displayMask);
		return dialog.open();
	}
	 ...
	 *            The list to fill.
	 */
	private void populateList(Text textToPopulate) {
		populateList(textToPopulate, status, 0, shouldIncludeTopLevelErrorInDetails);
	}
	 ...
	 *            its children
	 */
	private void populateList(Text textToPopulate, IStatus buildingStatus, int nesting, boolean includeStatus) {

		if (!buildingStatus.matches(displayMask)) {
			return;
		}

		textToPopulate.setText(getDetails(buildingStatus));
	}
